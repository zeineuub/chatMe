...
class User(Base):
    ...
    def pending_requests(self):
        return [
            friendship
            for friendship in self.received_requests
            if friendship.status == Status.PENDING.value
        ]
    ...
    def friends(self):
        return [
            friendship
            for friendship in self.sent_requests + self.received_requests
            if friendship.status == Status.CONFIRMED
        ]
...
class Friendship(Base):
    ...
    chat_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("chats.id"), unique=True
    )
    chat: Mapped[Optional["Chat"]] = relationship(
        "Chat", back_populates="friendship", uselist=False
    )
...
class Chat(Base):
    ...
    friendship: Mapped["Friendship"] = relationship(
        "Friendship", back_populates="chat", uselist=False
    )
